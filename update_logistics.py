...
def write_row(ws, row_num, row_data):
    ...
    for c_idx, cd in enumerate(row_data):
        ...
        if cd.get("comment"):    cell.comment       = cd["comment"]


def shift_data_validations(ws, insertions):
    """
    Update data-validation sqref ranges after rows have been inserted.

    insertions: list of 1-based original row numbers after which a new row was
                inserted (duplicates allowed when multiple rows follow the same
                source row).

    For each validation range [r1, r2]:
      - Rows strictly above r1 shift the entire range up.
      - Rows within [r1, r2] extend r2, keeping the newly inserted row inside
        the same validation as its source.
    """
    if not insertions:
        return
    ...
    for dv in ws.data_validations.dataValidation:
        ...
        for cr in list(dv.sqref.ranges):
            ...
            new_r1 = r1 + sum(1 for ins in insertions if ins < r1)
            new_r2 = r2 + sum(1 for ins in insertions if ins <= r2)

            col1 = get_column_letter(c1)
            col2 = get_column_letter(c2)
            if c1 == c2 and new_r1 == new_r2:
                new_parts.append(f"{col1}{new_r1}")
            else:
                new_parts.append(f"{col1}{new_r1}:{col2}{new_r2}")
...
def build_inserted_row(source_row, tracking, invoice_no, invoice_date, amount, max_col):
    ...
    for c in range(1, max_col + 1):
        ...
        new_row.append({
            ...
            "comment":       None,   # new inserted row inherits no notes
        })
    return new_row
...
def update_sheet(ws, invoice_no, invoice_date, shipments):
    ...
    for r in range(2, ws.max_row + 1):
        ...
        if val and str(val).strip() not in ("(Tracking link)", ""):
            key = normalize_tracking(val)
            u_empty = ws.cell(row=r, column=COL_U).value is None
            # Prefer rows where U is empty: if the tracking already maps to a
            # filled row but this row has U empty, this one should be filled first.
            if key not in existing_rows or u_empty:
                existing_rows[key] = r
    ...
    if to_insert:
        ...
        shift_data_validations(ws, [row_idx for _, _, row_idx in to_insert])
